...
def output_format_contract() -> dict:
    return {
        ...
        "description_format": "Обычная проза plain text; курсив только для коротких ремарок/физических деталей.",
        "rules": [
            ...
            "No empty scenes: every scene needs a hook, conflict, conversation, observation, social reaction, rumor, consequence, or time movement.",
            "Academy is a visual-novel scene, not a dry checklist; resolve routine movement to the nearest meaningful beat.",
            "Loaded character ids are internal; use visible descriptors until POV has name source.",
            "Livia is Akira's close school friend for about six years, not a new roommate.",
            "Roommates, named NPCs, conflicts, character rotation and knowledge sources must persist through state/story/knowledge/session_npcs.",
            "Events, obligations and dated memories must persist in state/story_lines.json; do not create one-scene state files.",
            "Check gpt/scene_format.md and runtime_scene_rules_digest.md; if any line violates them, rewrite before sending.",

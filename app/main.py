...
def apply_turn_result(session_id: str, request: ApplyTurnResultRequest) -> ApplyTurnResultResponse:
    ...
    # State advances only through explicit structured changes from the turn result.
    # Do not infer progress from rendered scene text: the text may contain unchosen UI options.
    current_state_changes = request.current_state_changes
    calendar_runtime_changes = request.calendar_runtime_changes
